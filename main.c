...
#define MAX_TASKS                           12
#define MAX_MUTEXES                         11

#define CONTROL_HEATER_PERIODICITY          200U
...
#define RUNTIME_MEASUREMENTS_PERIODICITY    4000U
...
void vControlHeater(void *pvParameters)
{
    ...
    for(;;)
    {
        ...
        if(xSemaphoreTake(xBtnSemaphore[seat_id], 0) == pdTRUE)
        {
            if(xSemaphoreTake(xDesiredHeaterLvlMutex[seat_id], 0) == pdTRUE)
            {
                ...
                if(xSemaphoreTake(xTempMutex[seat_id], 0) == pdTRUE)
                {
...
void vDisplayDataHandler(void *pvParameters)
{
    ...
    for(;;)
    {
        if(xSemaphoreTake(xUART_Mutex, portMAX_DELAY) == pdTRUE)
        {
            ...
            UART0_SendString("\r\nHeater Status: ");
            if(xSemaphoreTake(xHeaterStatusMutex[seat_id], 0) == pdTRUE)
            {
...
void vDiagnostics(void *pvParameters)
{
    ...
    for(;;)
    {
        ...
        if(xSemaphoreTake(xLastHeatLvlMutex[seat_id], 0) == pdTRUE)
        {
...
void vRuntimeMeasurements(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        for(ucCounter = 1; ucCounter < MAX_TASKS; ucCounter++)
        {
            ...
            strcpy(str, "");
            uint32_t val = ullTasksExecutionTime[ucCounter];
            sprintf(str, "%lu.%lu", val/10, val%10);
        ...
        for(ucCounter = 0; ucCounter < MAX_MUTEXES; ucCounter++)
        {
            ...
            UART0_SendString(" ticks / ");
            strcpy(str, "");
            uint32_t val = mutexStats[ucCounter].totalLockTime;
            sprintf(str, "%lu.%lu", val/10, val%10);
            UART0_SendString(str);
            UART0_SendString(" ms\r\n* ");
        }
...
void  GPIOPortF_Handler(void)
{
    ...
    if(GPIO_PORTF_RIS_REG & (1<<4))
    {
        ...
        if(xSemaphoreTakeFromISR(xLastHeatLvlMutex[0], 0) == pdTRUE)
        {
    ...
    if(GPIO_PORTF_RIS_REG & (1<<0))
    {
        ...
        if(xSemaphoreTakeFromISR(xLastHeatLvlMutex[1], 0) == pdTRUE)
        {
